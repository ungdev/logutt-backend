  ...
  ObjectInstance.create(objectInstance, (err, data) => {
    if (err)
      res.status(500).send({
        message:
          err.message ||
          'Some error occurred while creating the ObjectInstance.',
      });
    else res.send(data);
  });
};
...
exports.update = (req, res) => {
  ...
  ObjectInstance.updateById(
    req.params.objectInstanceId,
    req.body,
    (err, data) => {
      if (err) {
        if (err.kind === 'not_found') {
...
exports.delete = (req, res) => {
  ObjectInstance.remove(req.params.objectInstanceId, (err) => {
    if (err) {
      if (err.kind === 'not_found') {
        res.status(404).send({
          message: `Not found ObjectInstance with id ${req.params.objectInstanceId}.`,
